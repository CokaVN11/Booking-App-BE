...
export class RoomTypeService {
  getRoomTypeIdByName = async (name: string) => {
    try {
      const room_type = await RoomTypeModel.findOne({ name });
      if (!room_type) {
        return null;
      }
  ...
  addRoomType = async (room_type_data: RoomType) => {
    const roomTypeExist = await RoomTypeModel.findOne({
      hotel: room_type_data.hotel,
  ...
  updateRoomType = async (room_type_id: string, room_type_data: RoomType) => {
    try {
      const room_type = await RoomTypeModel.findByIdAndUpdate(
        room_type_id,
        room_type_data,
        { new: true }
      );
