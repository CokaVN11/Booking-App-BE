...
export class RoomService {
  ...
  addRoom = async (room_data: Room) => {
    const hotel_id = await AccountService.getInstance().getHotelIdByName(
      room_data.hotel
    );
    // Check exist
    ...
    const room_type_id =
      await RoomTypeService.getInstance().getRoomTypeIdByName(
        room_data.room_type
      );
  ...
  getAmenitiesByHotelId = async (hotel_id: string) => {
    try {
      ...
      const data = await AmenityService.getInstance().getListAmenity(
        uniqueAmenities
      );
